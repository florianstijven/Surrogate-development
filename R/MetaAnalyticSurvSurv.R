...
#'
#'The function 'MetaAnalyticSurvSurv_MA()' fits the model for a time-to-event surrogate and time-to-event true endpoint developed by Burzykowski et al. (2001) in the meta-analytic multiple-trial setting.
#'
#' @details
#'
#' # Model
...
MetaAnalyticSurvSurv <- function(data, true, trueind, surrog, surrogind,
  ...
  output_list <- list(stage1 = stage1,
                      stage2 = stage2)
  return(output_list)
...
MetaAnalyticSurvSurv_clayton <- function(data_m) {
  ...
  output_list <- list(alpha_delta_df = alpha_delta_df,
                      tau_delta_df = tau_delta_df, memo=memo, nlm_output=nlm_output)
  return(output_list)
...
MetaAnalyticSurvSurv_plackett <- function(data_m) {
  ...
  output_list <- list(theta_df = theta_df, rho_df=rho_df, memo=memo, fit_output=nlm_output)
  return(output_list)
...
trial_level_secondstage_unadjusted <- function(memo) {
  ...
  Trial.R2.unadj <- data.frame(cbind(R2, lo_R2, up_R2), stringsAsFactors = TRUE)
  colnames(Trial.R2.unadj) <- c("R2 Trial (unadjusted)", "CI lower limit",
                                "CI upper limit")
  rownames(Trial.R2.unadj) <- c(" ")


  return(list(Trial.R2.unadj=Trial.R2.unadj))
...
trial_level_secondstage_weighted <- function(memo) {
  ...
  colnames(Trial.R2.weighted) <- c("R2 Trial (weighted)", "CI lower limit",
                                   "CI upper limit")
  rownames(Trial.R2.weighted) <- c(" ")

  return(list(Trial.R2.weighted=Trial.R2.weighted))
...
trial_level_secondstage_adjusted <- function(memo) {
  ...
  Trial.R2.adj <- data.frame(cbind(R2, lo_R2, up_R2), stringsAsFactors = TRUE)

  colnames(Trial.R2.adj) <- c("R2 Trial (adjusted)", "CI lower limit",
                              "CI upper limit")
  rownames(Trial.R2.adj) <- c(" ")

  return(list(Trial.R2.adj=Trial.R2.adj))
