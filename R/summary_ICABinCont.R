summary.ICA.BinCont <- function(object, ..., Object){
  ...
  cat("\n#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n")
  cat("Mean (SD) R2_H: ",
      format(round(mean(Object$R2_H), 4), nsmall = 4),
      " (",
      format(round(sd(Object$R2_H), 4), nsmall = 4),
      ")")
  cat("\nMode R2_H: ", format(round(mode(Object$R2_H)$mode_val, 4), nsmall = 4))
  cat("\nMedian R2_H: ", format(round(quantile(Object$R2_H, probs = 0.50)), nsmall = 4))
  cat("\nRange R2_H: ", format(round(range(Object$R2_H), 4), nsmall = 4))
  cat("\n\nQuantiles of the R2_H distribution: \n\n")
  quant <-
    quantile(Object$R2_H, probs = c(.05, .10, .20, .50, .80, .90, .95))
  print(quant)
}
