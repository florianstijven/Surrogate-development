...
#  }
  ...
  if (R2_H==TRUE){
    ...
    if (Type=="All.Densities"){
      ...
      max_val <- max(max(density(R2_H_General, na.rm = T)$y), max(density(R2_H_No, na.rm = T)$y),  max(density(R2_H_Surr, na.rm = T)$y), 
                     max(density(R2_H_True, na.rm = T)$y),  max(density(R2_H_SurrTrue, na.rm = T)$y))   
  ...
  if (R_H==TRUE){
    ...
    if (Type=="All.Densities"){
      ...
      R2_H_True <- resul$R2_H[resul$Monotonicity=="True"]
      R2_H_SurrTrue <- resul$R2_H[resul$Monotonicity=="SurrTrue"]
      max_val <- max(max(density(sqrt(R2_H_General), na.rm = T)$y), max(density(sqrt(R2_H_No), na.rm = T)$y),  max(density(sqrt(R2_H_Surr), na.rm = T)$y),
                     max(density(sqrt(R2_H_True), na.rm = T)$y),  max(density(sqrt(R2_H_SurrTrue), na.rm = T)$y))   
      
      plot(density(sqrt(x$R2_H), na.rm = T), xlab=Xlab.R_H, ylab="Density", main=Main.R_H, lwd=2, ylim = c(0, max_val), col=0)
      try(lines(density(sqrt(R2_H_No), na.rm = T), lty=1, col=1, lwd=3), silent=TRUE) 
